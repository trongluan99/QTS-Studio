...
~~~
    ...
    implementation 'com.github.trongluan99:Qts-Studio:1.0'
...
# <a id="setup_Qtsad"></a>Setup QtsAd
...
class App extends AdsMultiDexApplication(){
    ...
    public void onCreate() {
        super.onCreate();
    ...
        String environment = BuildConfig.build_debug ? QtsAdConfig.ENVIRONMENT_DEVELOP : QtsAdConfig.ENVIRONMENT_PRODUCTION;
        ...
        qtsAdConfig.setAppsflyerConfig(appsflyerConfig);
    
        // Optional: setup client token SDK Facebook
        qtsAdConfig.setFacebookClientToken(FACEBOOK_CLIENT_TOKEN)
...
~~~ 
    QtsAdCallback adCallback = new QtsAdCallback() {
        @Override
...
~~~
    QtsAd.getInstance().setInitCallback(new QtsInitCallback() {
            @Override
            public void initAdSuccess() {
                QtsAd.getInstance().loadSplashInterstitialAds(SplashActivity.this, idAdSplash, TIME_OUT, TIME_DELAY_SHOW_AD, true, adCallback);
            }
        });    
~~~
...
~~~
    private fun loadInterCreate() {
      ApInterstitialAd mInterstitialAd = QtsAd.getInstance().getInterstitialAds(this, idInter);
    }
~~~
...
~~~
    <com.ads.control.ads.bannerAds.QtsBannerAdView
...
~~~~
    QtsAd.getInstance().loadBanner(this, idBanner);
~~~~
...
~~~~
    QtsAd.getInstance().loadNativeAdResultCallback(this,ID_NATIVE_AD, com.ads.control.R.layout.custom_native_max_small,new QtsAdCallback(){
            @Override
...
~~~~
    QtsAd.getInstance().populateNativeAdView(MainApplovinActivity.this,nativeAd,flParentNative,shimmerFrameLayout);
~~~~
...
~~~~
    <com.ads.control.ads.nativeAds.QtsNativeAdView
        android:id="@+id/QtsNativeAds"
        android:layout_width="match_parent"
...
~~~~
    QtsNativeAdView.loadNativeAd(this, idNative);
~~~~
...
~~~~
    ...
    QtsAdAdapter adAdapter = QtsAd.getInstance().getNativeRepeatAdapter(this, idNative, layoutCustomNative, com.ads.control.R.layout.layout_native_medium, originalAdapter, listener, 4);
    
    // Ad native fixed in position
    QtsAdAdapter adAdapter = QtsAd.getInstance().getNativeFixedPositionAdapter(this, idNative, layoutCustomNative, com.ads.control.R.layout.layout_native_medium, originalAdapter, listener, 4);
...
~~~~
    ApRewardAd rewardAd = QtsAd.getInstance().getRewardAd(this, idAdReward);
...
~~~~
    override fun onCreate() {
      ...
      qtsAdConfig.setIdAdResume(AppOpenManager.AD_UNIT_ID_TEST);
      ...
    }
~~~~
